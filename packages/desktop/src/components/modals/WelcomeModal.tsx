...
export function WelcomeModal({
  ...
  return (
    ...
    >
      <div
        className="bg-slate-800 rounded-2xl p-6 max-w-2xl w-full border border-slate-700 max-h-[90vh] overflow-y-auto relative"
        ...
        <div className="space-y-6">
          <div className="text-center">
            ...
            <p className="text-slate-400 text-lg">
              {t('welcome.subtitle', "Let's set up your shared data file")}
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-slate-700/30 rounded-xl p-4 border border-slate-600/50">
              ...
              <p className="text-sm text-slate-400">
                {t('welcome.benefit1Desc', 'Both partners can open the same shared JSON file')}
              </p>
            </div>
            ...
            <div className="bg-slate-700/30 rounded-xl p-4 border border-slate-600/50">
              ...
              <p className="text-sm text-slate-400">
                {t('welcome.benefit2Desc', 'Stored in your own shared cloud folder')}
              </p>
            </div>

            <div className="bg-slate-700/30 rounded-xl p-4 border border-slate-600/50">
              <CheckCircle className="w-8 h-8 text-purple-400 mb-2" />
              <h3 className="font-semibold text-white mb-1">{t('welcome.benefit3Title', 'Safer Sync')}</h3>
              <p className="text-sm text-slate-400">
                {t('welcome.benefit3Desc', 'The app warns before stale writes overwrite newer shared data')}
              </p>
            </div>
          </div>
          ...
          {suggestedClouds.length > 0 && (
            <div>
              <h3 className="text-sm font-semibold text-slate-400 mb-3">
                {t('welcome.suggestedClouds', 'Create a new shared file in one of these folders')}
              </h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 border-t border-slate-700 pt-4">
            <Button onClick={onChooseCustomFolder} variant="secondary" className="w-full">
              <FolderOpen className="w-4 h-4 mr-2" />
              {t('welcome.chooseCustomFolder', 'Create shared file somewhere else')}
            </Button>
            <Button onClick={onOpenExistingFile} variant="secondary" className="w-full">
              <FolderOpen className="w-4 h-4 mr-2" />
              {t('welcome.openExistingFile', 'Open existing shared file')}
            </Button>
          </div>
